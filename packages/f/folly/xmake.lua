package("folly")
    ...
    add_patches("2025.07.28", path.join(os.scriptdir(), "patches", "2025.07.28", "fmt12.patch"), "a568f8562b4b285e57c6089a7e359187dd08f20d2ee61712bcbcfe4aa6488a37")
    ...
    add_deps("double-conversion", "fast_float", "gflags", "glog", "zlib", "fmt")
    ...
    on_load(function (package)
        ...
        if package:is_plat("linux") and package:config("liburing") then
            if package:version():ge("2025.02.24") then
                package:add("deps", "liburing >=2.10")
            else
    ...
    on_install("windows|x64", "macosx", "linux", "cross", function (package)
        ...
        if package:is_plat("cross") then
            ...
            table.insert(configs, "-DFOLLY_HAVE_UNALIGNED_ACCESS=ON")
            table.insert(configs, "-DFOLLY_HAVE_WEAK_SYMBOLS=ON")
            table.insert(configs, "-DFOLLY_HAVE_LINUX_VDSO=ON")
            table.insert(configs, "-DFOLLY_HAVE_WCHAR_SUPPORT=ON")
            table.insert(configs, "-DHAVE_VSNPRINTF_ERRORS=ON")
        end
